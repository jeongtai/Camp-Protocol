...
const main = async () => {
  ...
  //SWAP
  while (1) {
    try {const swapamounts = await router.getAmountsOut(200e6, [KUSDT, KROME]);
    ...
    await router.swapExactTokensForTokens(swapamounts[0].toString(), swapamounts[1].toString(), [KUSDT, KROME], owner.address, Math.floor(Date.now()) + 10);
    console.log(i)
