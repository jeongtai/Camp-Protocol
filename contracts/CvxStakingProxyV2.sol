...
// send rewards back to owner(kp locker)
...
contract KpStakingProxyV2 {
    ...
    function getBalance() external view returns(uint256) {
        return IConvexRewards(kpStaking).balanceOf(address(this));
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        //unstake
        IConvexRewards(kpStaking).withdraw(_amount, false);
    ...
    function stake() external {
        ...
        IConvexRewards(kpStaking).stakeAll();
    }
    ...
    function distribute() external {
        ...
        //claim rewards
        IConvexRewards(kpStaking).getReward(false);
        ...
        uint256 sCheck  = IConvexRewards(kpEKLStaking).balanceOf(address(this));
        if(sCheck > 0){
            IConvexRewards(kpEKLStaking).withdraw(sCheck,false);
        }
